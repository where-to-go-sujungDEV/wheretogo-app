...
export const getTopContents = (result) => { 
    db.query("select eventID, eventName, startDate, w1+w2+w3+w4+w6+m1+m2+m3+m4+m6 as totalSavedNum, endDate, kind, pic from eventTBL ORDER BY totalSavedNum DESC LIMIT 5;", (err, results) => {             
        if(err) {
            result(500, err, null);
        } else {
            result(200, null, {
...
export const getUserTopContents = (uid, result) => { 
    db.query("select sex, age from userTBL where userID = ?;",[uid], (err, userInfo) => {             
        if(err) {
            ...
            var qr = 'select eventID,eventName,startDate, endDate, kind, pic, ';  
...
export const getEventByEventID = (id, result) => { 
    db.query("select eventID,eventName,startDate, w1+w2+w3+w4+w6+m1+m2+m3+m4+m6 as savedNum , endDate, kind, pic, areacode, sigungucode, addr1 as place, addr2 as detailedPlace, mapx,mapy,mlevel,tel, telname, homepage, overview, eventplace,bookingplace, subevent, price from eventTBL where eventID = ?;", [id], (err, results) => {             
        if(err) {
            result(500, err, null);
        } else {
            result(200, null, {
